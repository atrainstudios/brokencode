...
var init = () => {
    ...
    // c1
    {
        ...
        c1 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(15, 0)));
    ...
    // c2
    {
        ...
        c2 = theory.createUpgrade(1, currency, new ExponentialCost(5, 0));
    ...
    {
        c2Exp = theory.createMilestoneUpgrade(1, 1000);
...
var tick = (elapsedTime, multiplier) => {
    ...
    currency.value += dt * bonus * BigNumber.from(getC1(c1.level)).pow(getC1Exponent(c1Exp.level)) *
                                   BigNumber.from(getC2(c2.level)).pow(getC2Exponent(c2Exp.level));
}
...
var getPublicationMultiplier = (tau) => tau.pow(2.64) / BigNumber.THREE;
var getPublicationMultiplierFormula = (symbol) => "\\frac{{" + symbol + "}^{0.164}}{3}";
var getTau = () => currency.value;
var get2DGraphValue = () => currency.value.sign * (BigNumber.ONE + currency.value.abs()).log10().toNumber();

var getC1 = (level) => Utils.getStepwisePowerSum(level, 2, 10, 0);
var getC2 = (level) => BigNumber.from(level).pow(level);
