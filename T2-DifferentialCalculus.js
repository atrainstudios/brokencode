...
var id = "differential_calculus_broken"
var name = "Totally Normal Differential Calculus";
var description = "An implementation of the 'Differential Calculus' theory from the game.";
var authors = "Gilles-Philippe Paillé";
var version = 69;

var currency;
var q1 = BigNumber.TWO, q2 = BigNumber.ONE, q3 = BigNumber.ONE, q4 = BigNumber.ONE;
...
var init = () => {
    ...
    // q1
    {
        ...
        dq1 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(10, Math.log2(1.9))));
    ...
    // q2
    {
        ...
        dq2 = theory.createUpgrade(1, currency, new ExponentialCost(5000, Math.log2(1.9)));
    ...
    // q3
    {
        ...
        dq3 = theory.createUpgrade(2, currency, new ExponentialCost(3e5, Math.log2(2)));
    ...
    // q4
    {
        ...
        dq4 = theory.createUpgrade(3, currency, new ExponentialCost(8e5, Math.log2(2)));
    ...
    // r1
    {
        ...
        dr1 = theory.createUpgrade(4, currency, new ExponentialCost(2e6, Math.log2(1.8)));
    ...
    // r2
    {
        ...
        dr2 = theory.createUpgrade(5, currency, new ExponentialCost(3e9, Math.log2(1.9)));
    ...
    // r3
    {
        ...
        dr3 = theory.createUpgrade(6, currency, new ExponentialCost(4e5, Math.log2(2)));
    ...
    // r4
    {
        ...
        dr4 = theory.createUpgrade(7, currency, new ExponentialCost(5e5, Math.log2(2)));
...
var tick = (elapsedTime, multiplier) => {
    ...
    q2 = q2 + dt * getDQ2(dq2.level) * (qTerms.level > 0 ? q3 : BigNumber.TWO);
    if (qTerms.level > 0) q3 = q3 + dt * getDQ3(dq3.level) * (qTerms.level > 1 ? q4 : BigNumber.TWO);
    if (qTerms.level > 1) q4 = q4 + dt * getDQ4(dq4.level);

    r1 = r1 + dt * getDR1(dr1.level) * r2;
    r2 = r2 + dt * getDR2(dr2.level) * (rTerms.level > 0 ? r3 : BigNumber.THREE);
    if (rTerms.level > 0) r3 = r3 + dt * getDR3(dr3.level) * (rTerms.level > 1 ? r4 : BigNumber.TEN);
    if (rTerms.level > 1) r4 = r4 + dt * getDR4(dr4.level);

    currency.value += bonus * dt * (q1.pow(getQ1Exp(q1Exp.level)) *
                                    r1.pow(getR1Exp(r1Exp.level)))*dt;
...
var getPrimaryEquation = () => {
    ...
    if (q1Exp.level == 1) result += "^{1.35}";
    if (q1Exp.level == 2) result += "^{1.7}";
    if (q1Exp.level == 3) result += "^{2.05}";
    result += "r_1";
    if (r1Exp.level == 1) result += "^{1.35}";
    if (r1Exp.level == 2) result += "^{1.7}";
    if (r1Exp.level == 3) result += "^{2.05}";
...
var getPublicationMultiplier = (tau) => tau.pow(0.248) / BigNumber.HUNDRED;
var getPublicationMultiplierFormula = (symbol) => "\\frac{{" + symbol + "}^{0.248}}{100}";
var getTau = () => currency.value;
var get2DGraphValue = () => currency.value.sign * (BigNumber.TWO + currency.value.abs()).log10().toNumber();
...
var getQ1Exp = (level) => BigNumber.from(1 + level * 0.35);
var getR1Exp = (level) => BigNumber.from(1 + level * 0.35);
